...
pub async fn start_timer(
    ...
    // 1. Check if there's already an active timer — stop it first
    let existing: Option<(String, String, String, String)> = query_as(
        "SELECT id, task_id, start_time, notes FROM active_timers LIMIT 1"
    )
    ...
    if let Some((timer_id, old_task_id, start_time_str, old_notes)) = existing {
        ...
        query(
            "INSERT INTO time_entries (id, task_id, start_time, end_time, duration_minutes, notes, created_at) VALUES ($1,$2,$3,$4,$5,$6,$7)"
        )
        .bind(&entry_id).bind(&old_task_id).bind(&start_time_str).bind(&now_str).bind(duration_minutes).bind(&old_notes).bind(&now_str)
    ...
    query(
        "INSERT INTO active_timers (id, task_id, start_time, notes, created_at) VALUES ($1,$2,$3,$4,$5)"
    )
    .bind(&id).bind(&task_id).bind(&now_str).bind(&notes).bind(&now_str)
    .execute(pool.get_ref()).await.unwrap();

    // Set task to in_progress
    query("UPDATE tasks SET status = 'in_progress', updated_at = $1 WHERE id = $2")
...
pub async fn stop_timer(pool: web::Data<DbPool>) -> impl Responder {
    let now = Utc::now();
    let now_str = now.to_rfc3339();
    ...
    match existing {
        Some((timer_id, task_id, start_time_str, notes)) => {
            ...
            .bind(&entry_id).bind(&task_id).bind(&start_time_str).bind(&now_str).bind(duration_minutes).bind(&notes).bind(&now_str)
            .execute(pool.get_ref()).await.ok();
...
pub async fn get_active_timer(pool: web::Data<DbPool>) -> impl Responder {
    let now = Utc::now();
